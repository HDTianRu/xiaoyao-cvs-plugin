...
export default class user {
  ...
  async seachUid (data) {
    ...
    if (data?.data) {
      ...
      logger.mark(`绑定stoken成功 [ltuid:${this.e?.sk?.get('stuid') || this.e.stuid}]`)
      let msg = `米游社UID: ${this.e?.sk?.get('stuid') || this.e.stuid}`
  ...
  async delSytk (path = yamlDataUrl, e, type = "stoken") {
    ...
    fs.exists(file, (exists) => {
      ...
      if (ck?.yuntoken) {
        ...
        if (user.ltuid) {
          if (!Object.values(ck).some(i => Number(i.stuid) === Number(user.ltuid))) {
            e.reply('当前UID未绑定stoken')
            return true
          }
          lodash.forEach(ck, (v, i) => {
            if (Number(v.stuid) === Number(user.ltuid)) {
              delete ck[i];
            }
          })
        } else {
          ...
          let sk = ck[e.uid]
          lodash.forEach(ck, (v, i) => {
            if (Number(sk?.stuid) === Number(v?.stuidn)) {
              delete ck[i];
            }
          })
        }
